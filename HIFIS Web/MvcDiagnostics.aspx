...
<script runat="server">
    private static readonly DateTime _utilityDate = new DateTime(2018, 4, 13);
    private static readonly string _utilityVersion = "v12A";
    ...
    private static readonly MvcCoreAssemblyInfo[] _mvcCoreAssemblyHistory = new MvcCoreAssemblyInfo[] {
        ...
        new MvcCoreAssemblyInfo() { Version = "4.0.20710.0",     Name = "ASP.NET MVC 4 RTM", FuturesVersion = "4.0.20710.0" },
        new MvcCoreAssemblyInfo() { Version = "4.0.40804.0",     Name = "ASP.NET MVC 4 RTM (KB2993928)", FuturesVersion = "4.0.20710.0" },
        ...
        new MvcCoreAssemblyInfo() { Version = "5.2.60315.0", Name = "ASP.NET MVC 5.2.5 PREVIEW1" },
    };
    ...
    private static readonly MvcFuturesAssemblyInfo[] _mvcFuturesAssemblyHistory = new MvcFuturesAssemblyInfo[] {
        ...
        new MvcFuturesAssemblyInfo() { Version = "3.0.11029.0", Name = "ASP.NET MVC 3 RC 1 Futures" },
        new MvcFuturesAssemblyInfo() { Version = "3.0.11209.0", Name = "ASP.NET MVC 3 RC 2 Futures" },
        new MvcFuturesAssemblyInfo() { Version = "3.0.20105.0", Name = "ASP.NET MVC 3 RTM Futures", DownloadUrl = "http://aspnet.codeplex.com/downloads/get/211128" },

        // v4
        new MvcFuturesAssemblyInfo() { Version = "4.0.20710.0", Name = "ASP.NET MVC 4 RTM Futures", DownloadUrl = "https://www.nuget.org/packages/Mvc4Futures/4.0.20710" },
    };
...
<body>
    ...
    <div class="box">
        <p>
            ...
            <b>.NET CLR version:</b> <%= E(results.EnvironmentInformation.NetFrameworkVersion) %> (<%= E(results.EnvironmentInformation.NetFrameworkBitness) %>-bit)<br />
            <b>.NET Framework version:</b> <%= E(x.Get45PlusFromRegistry()) %><br />
            ...
            <b>Application Windows Identity:</b> <%= E(System.Security.Principal.WindowsIdentity.GetCurrent().Name) %><br />
    ...
    <h2>Crystal Reports and SQL Drivers</h2>
    <div class="box">
        <%
            RegistryView registryView = Environment.Is64BitOperatingSystem ? RegistryView.Registry64 : RegistryView.Registry32;
            using (RegistryKey hklm = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, registryView))
            {
                RegistryKey crystal = hklm.OpenSubKey(@"SOFTWARE\SAP BusinessObjects\Crystal Reports for .NET Framework 4.0\Crystal Reports", false);
                RegistryKey sqlcn10 = hklm.OpenSubKey(@"SOFTWARE\Classes\SQLNCLI10", false);
                RegistryKey sqlcn11 = hklm.OpenSubKey(@"SOFTWARE\Classes\SQLNCLI11", false);
        %>

        <% if (crystal == null)
            { %>
        <p>
            A Crystal Reports Runtime Engine has not been loaded into this application.
            <br />
            If desired, you can download a Crystal Reports Runtime Engine from <a href="">HIFIS Support</a>.
        </p>
        <% } %>
        <p>
            <% if (sqlcn10 != null)
                { %> <b>SQL Driver:</b> <%= sqlcn10.GetValue("") %>
            <br />
            <% } %>
            <% if (sqlcn11 != null)
                { %> <b>SQL Driver:</b> <%= sqlcn11.GetValue("") %>
            <br />
            <% } %>
            <% if (crystal != null)
                {
                    try
                    {
                        foreach (var item in crystal.GetValueNames())
                        {
                            if (item.ToLower().Contains("version"))
                            {
                                if (item.ToLower().Contains("64"))
                                { %>
                                    <b>Crystal Reports Runtime Engine:</b> <%= " Version " + crystal.GetValue(item) + " (64-bit)" %> <br />
                            <%  } else { %>
                                    <b>Crystal Reports Runtime Engine:</b> <%= " Version " + crystal.GetValue(item) %>
                         <% }
                    }
                }
            }
    ...
    <div class="box">
        ...
        <table>
            ...
            <tbody>
                <% foreach (Assembly assembly in results.AllAssemblies) { %>
                    <tr style="text-align: left; vertical-align: top;">
                        <td><%= E(assembly.GetName().Name) %></td>
                        <td><%= E(assembly.GetName().Version) %></td>
                        <td><%= E(String.Concat(assembly.GetName().GetPublicKeyToken().Select(b => String.Format("{0:x2}", b)))) %></td>
                        <% var l = new Uri(assembly.GetName().CodeBase).LocalPath; %>
                        <td style="text-wrap: none;"><%= E(l) %></td>
                    </tr>
                <% } %>
            </tbody>
        </table>
    ...
    <div class="box">
        ...
        <% 
            ...
            foreach (ChannelEndpointElement el in clientSection.Endpoints)
            {
                string addr = el.Address.ToString(); %> 
            <tr style="text-align: left;">
                <td><a href="<%= E(addr) %>"><%= E(addr) %></a></td>
                ...
                <%  
                    ...
                    try
                    {
                        //HttpWebResponse response = (HttpWebResponse)request.GetResponse();
                            %><td><%--<%= E(response.StatusDescription) %>--%></td> <%
                        }
...
<script runat="server">
    ...
    {
        public string Get45PlusFromRegistry()
        {
            string version = ".NET Framework Version 4.5 or later is not detected.";
            ...
            using (var ndpKey = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry32).OpenSubKey(subkey))
            {
                ...
                {
                    var v = CheckFor45PlusVersion((int)ndpKey.GetValue("Release"));
                    version = v;
                }
            }
        ...
        public string CheckFor45PlusVersion(int releaseKey)
        {
